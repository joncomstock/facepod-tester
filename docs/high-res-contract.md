...
- A handle `id` can also be evicted by **cap pressure** (the buffer keeps only the most recent few stills) or by **TTL**, not only by fetch — a `downloadHighResImage` on an evicted id returns the standard `404` envelope (typed `ApiError` client-side).
